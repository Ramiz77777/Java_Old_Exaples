package zadacijavarash.uroven12zadaca2;
 ...
 */
public class PerevoracivaniyeDannix {
    public static ArrayList<Integer> numbers = new ArrayList<Integer>();           //int[] numbers = new int [10];
                                                                                                // ne reshil
    ...
    public static void init() {
        for (int i = 0; i < 10; i++) {
            numbers.add(i);                                  //numbers[i] = i;
        }
    }

    public static void reverse() {
        int n = numbers.size() - 1;                    //int n = numbers.length - 1;
        for (int i = 0; i < numbers.size() / 2; i++) { // for (int i = 0; i < numbers.length / 2; i++) {
            int temp = numbers.get(i);                      //int temp = numbers[i];
            numbers.set(i,numbers.get(n-i));                //numbers[i] = numbers[n - i];
            numbers.set(n-i,temp);                          //numbers[n - i] = temp;
        }
    }
